...
function renderRegular(){
  ...
  const cards=groups.map(group=>{
    ...
    const list=group.rows.length?`<div class="csl-result-list">${group.rows.map(x=>{
      const f=fit(x.diff),chance=engine.admissionChance(x.diff,{verified,useStd:x.useStd,quality:x.quality});
      return `<article class="csl-result"><div><div class="csl-result-name">${esc(x.u.n)}</div><div class="csl-result-dept">${esc(x.u.d)} · ${esc(x.u.g)}군 · ${esc(x.u.s)}</div><div class="csl-rule">${esc(x.u.sk||'대학별 환산 규칙 확인 필요')} · ${x.mode} · 반영 국 ${x.u.kw||0}% / 수 ${x.u.mw||0}% / 영 ${x.u.ew||0}% / 탐 ${x.u.sw||0}%${state.arts?' · 실기 반영비율·종목은 최종 모집요강 재확인':''}</div></div><div class="csl-result-side"><div class="csl-fit ${f[0]}">${f[1]}</div><div class="csl-number">${x.diff>=0?'+':''}${Math.round(x.diff)}</div><div class="csl-meta">대학별 ${x.useStd?'표준':'백분위'} 비교차</div><div class="csl-chance"><strong>추정 합격률 약 ${chance.point}%</strong><span>${chance.low}–${chance.high}% · 신뢰도 ${chance.confidence}</span><i><b style="width:${chance.point}%"></b></i></div></div></article>`;
  ...
  return `<div class="csl-note">${state.arts?'예체능':'일반'} 정시 라인 · 시나리오 표준합 ${Math.round(std)}점 / 백분위합 ${Math.round(pct)}점. 대학별 반영영역·탐구 수·영어 선택반영·필수 선택과목을 적용한 뒤 상향·소신·적정·안정별 상위 대학을 최대 8개씩 표시합니다. 합격률은 2025 입결 컷 대비 정규화 점수차를 보수적으로 환산한 상담 추정치이며 ${verified?'확인 점수 기준':'추정 점수·목표 시나리오 기준'}입니다. 경쟁률·충원·당해 지원 이동은 미반영이므로 범위를 함께 보세요. <a href="https://www.adiga.kr/ucp/uvt/uni/univDetailSelection.do?menuId=PCUVTINF2000&searchSyr=2022&unvCd=0000020" target="_blank" rel="noopener">어디가 70% cut 정의</a></div>${cards||'<div class="csl-empty">조건에 맞는 대학이 없습니다.</div>'}`;
}
