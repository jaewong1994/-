...
(async()=>{
 ...
 for(const role of ['student','teacher','director']){
  ...
  await page.setContent(`<body class="auth-pending"><section id="auth-gate"><p id="auth-status"></p><form id="auth-form"><input id="auth-email"><input id="auth-password"><button>로그인</button></form><button id="auth-retry"></button></section><div class="cat" data-cat="counsel"></div><div class="cat" data-cat="exam-counsel"></div><div class="cat" data-cat="admin"></div><div id="uni-content"></div>`);
  ...
  assert.equal(await page.evaluate(()=>AppSession.canOpen('exam-counsel')),role!=='student');
